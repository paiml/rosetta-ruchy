...
impl<T: Ord + Debug + Clone + Display> RedBlackTree<T> {
    ...
    fn insert_recursive(&mut self, node: Option<Box<RBNode<T>>>, value: T) -> Option<Box<RBNode<T>>> {
    ...
    fn display_node(&self, node: &Box<RBNode<T>>, prefix: &str, child_prefix: &str, is_last: bool) {
        ...
        for (i, child) in children.iter().enumerate() {
            ...
            self.display_node(child, &format!("{}", child_prefix), &new_prefix, is_last_child);
        }
    }
...
impl Display for TreeStatistics {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "   Theoretical max height: {:.1}", self.theoretical_max_height)?;
        writeln!(f, "   Height efficiency: {:.1}%", 
        ...
        writeln!(f, "   Operations per insert: {:.2}", 
                 if self.size > 0 {
...
fn benchmark_operations(size: usize) {
    ...
    println!("   Insert time: {:?} ({:.0} ops/sec)", 
             insert_time, size as f64 / insert_time.as_secs_f64());
    println!("   Search time: {:?} ({:.0} ops/sec)", 
             search_time, size as f64 / search_time.as_secs_f64());
...
fn stress_test_sequential() {
    ...
    println!("   Height: {} (theoretical max: {:.1})", 
             tree.height(), 2.0 * (size as f64 + 1.0).log2());
    println!("   Rotations: {}, Recolors: {}", 
...
fn main() {
    ...
    println!("Ascending pattern - Height: {}, Black height: {}", 
             ascending_tree.height(), ascending_tree.black_height());
    println!("Descending pattern - Height: {}, Black height: {}", 
             descending_tree.height(), descending_tree.black_height());
    
    ascending_tree.verify_invariants().expect("Ascending tree invariants violated!");
    descending_tree.verify_invariants().expect("Descending tree invariants violated!");
...
mod tests {
    ...
    fn test_sequential_insertion() {
        ...
        for i in 0..100 {
            tree.insert(i);
            assert!(tree.verify_invariants().is_ok(), 
                   "Invariants violated after inserting {}", i);
        }
