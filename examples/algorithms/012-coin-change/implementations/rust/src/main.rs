...
impl fmt::Display for CoinChangeResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.is_possible {
            ...
            writeln!(f, "  Total Value: {}", self.coins_used.iter().sum::<usize>())?;
        } else {
...
fn run_performance_comparison(coins: &[usize], amount: usize) {
    println!("Performance Comparison: coins={:?}, amount={}", coins, amount);
    println!("{}", "-".repeat(70));
    ...
    // Analyze results
    let optimal_results: Vec<_> = all_results.iter()
        .filter(|r| r.is_possible && (r.algorithm_used.contains("DP") || r.algorithm_used == "Naive Recursive"))
        .collect();
    
    if !optimal_results.is_empty() {
        let expected_coins = optimal_results[0].min_coins;
        let all_optimal_consistent = optimal_results.iter().all(|r| r.min_coins == expected_coins);
        println!("{}", "-".repeat(70));
        ...
        if let Some(greedy) = greedy_result {
            if greedy.is_possible {
                ...
                println!("Greedy algorithm optimal: {} (used {} coins)", 
...
fn run_test_case(name: &str, coins: Vec<usize>, amount: usize, expected_coins: Option<i32>) {
    ...
    if let Some(expected) = expected_coins {
        ...
        println!(
            "Expected: {}, Got: {}, Test: {}",
            if expected == -1 { "impossible".to_string() } else { expected.to_string() },
            if result.is_possible { result.min_coins.to_string() } else { "impossible".to_string() },
...
fn test_canonical_systems() {
    ...
    let test_cases = [
        ...
        ("Non-Canonical [1,3,4]", generate_coin_system("non_canonical"), 6),
    ...
    for (name, coins, amount) in test_cases {
        ...
        if dp_result.is_possible && greedy_result.is_possible {
            ...
            println!("  System is {}", if is_canonical { "CANONICAL" } else { "NON-CANONICAL" });
...
fn main() {
    ...
    if stress_result.is_possible {
        ...
        println!("Problems per second: {:.0}", (stress_amount as f64) / (stress_elapsed / 1000.0));
    }
    ...
    println!("Greedy Algorithm:   O(coins log coins) time, O(1) space, works for canonical systems");
