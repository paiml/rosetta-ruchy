...
impl DirectedGraph {
    ...
    fn add_vertex(&mut self, vertex: i32) {
        ...
        self.reverse_adjacency_list.entry(vertex).or_insert_with(Vec::new);
    }
    ...
    fn add_edge(&mut self, from: i32, to: i32) {
        ...
        self.reverse_adjacency_list.entry(to).or_insert_with(Vec::new).push(from);
        
        // Ensure both vertices have entries
        self.adjacency_list.entry(to).or_insert_with(Vec::new);
        self.reverse_adjacency_list.entry(from).or_insert_with(Vec::new);
    }
    ...
    fn get_in_degree(&self, vertex: i32) -> usize {
        self.reverse_adjacency_list.get(&vertex).map_or(0, |v| v.len())
    }
    ...
    fn has_cycle_util(&self, vertex: i32, visited: &mut HashSet<i32>, rec_stack: &mut HashSet<i32>) -> bool {
...
fn dfs_recursive_util(graph: &DirectedGraph, vertex: i32, visited: &mut HashSet<i32>, stack: &mut Vec<i32>) {
    visited.insert(vertex);
...
fn run_test_case(name: &str, graph: DirectedGraph) {
    ...
    if let Some(valid_result) = results.iter().find(|r| r.is_valid) {
        ...
        println!("\nValidation: {}", if is_valid { "✅ PASS" } else { "❌ FAIL" });
