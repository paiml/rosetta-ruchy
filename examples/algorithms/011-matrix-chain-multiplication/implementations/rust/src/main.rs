...
impl fmt::Display for MatrixChainResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "Matrix Chain Multiplication Result ({}):", self.algorithm_used)?;
        writeln!(f, "  Minimum Cost: {} scalar multiplications", self.min_cost)?;
...
fn matrix_chain_order_naive(dimensions: &[usize]) -> MatrixChainResult {
    ...
    MatrixChainResult::new(min_cost, "Not reconstructed".to_string(), "Naive Recursive", elapsed)
}
...
fn run_performance_comparison(dimensions: &[usize]) {
    println!("Performance Comparison for {} matrices:", dimensions.len() - 1);
    ...
    if dimensions.len() <= 7 {  // 6 matrices or fewer
...
fn generate_random_dimensions(count: usize, min_dim: usize, max_dim: usize, seed: u64) -> Vec<usize> {
    let mut rng = seed;
...
fn main() {
    ...
    println!("Memoized time: {:.2}ms", memoized_result.computation_time_ms);
    ...
    for n in 1..=10 {
        ...
        println!("{} matrices: {} ways to parenthesize (C_{})", n + 1, catalan, n);
    }
