...
fn run_performance_comparison(str1: &str, str2: &str) {
    ...
    for result in &all_results {
        println!("{:<20} | Distance: {:3} | Time: {:8.2}ms", 
...
fn run_test_case(name: &str, str1: &str, str2: &str, expected_distance: Option<usize>) {
    ...
    if let Some(expected) = expected_distance {
        ...
        println!("Expected: {}, Got: {}, Test: {}", 
                expected, result.distance, if passed { "PASS" } else { "FAIL" });
    }
...
fn main() {
    ...
    println!("Distance: {}, Time: {:.2}ms", large_result.distance, elapsed);
    println!("Mutation rate achieved: {:.1}%", (large_result.distance as f64 / 100.0) * 100.0);
    ...
    println!("Distance: {}, Time: {:.2}ms", stress_result.distance, stress_elapsed);
    println!("Operations per second: {:.0}", (1000.0 * 1000.0) / (stress_elapsed / 1000.0));
