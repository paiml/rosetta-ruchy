...
fn run_performance_comparison(prices: &[u32], length: usize) {
    println!("Performance Comparison: length={}, prices={:?}", length, prices);
    println!("{}", "-".repeat(70));
    ...
    if !optimal_results.is_empty() {
        ...
        let all_optimal_consistent = optimal_results.iter().all(|r| r.max_revenue == expected_revenue);
        
        println!("{}", "-".repeat(70));
        println!("Optimal algorithms consistent: {}", all_optimal_consistent);
        
        // Check greedy performance
        let greedy_result = all_results.iter().find(|r| r.algorithm_used == "Greedy Heuristic");
        if let Some(greedy) = greedy_result {
            ...
            println!(
                ...
                greedy.max_revenue, expected_revenue, approximation_ratio * 100.0
            );
        }
    }
}
...
fn analyze_cutting_efficiency(prices: &[u32]) {
    ...
    println!("{}", "-".repeat(40));
    println!("{:<8} {:<8} {:<12} {:<10}", "Length", "Price", "Unit Price", "Efficiency");
    ...
    for (length, price, unit_price) in &unit_prices {
        ...
        println!("{:<8} {:<8} {:<12.2} {:<10}", length, price, unit_price, efficiency);
    }
    ...
    if let Some((best_length, best_price, best_unit_price)) = unit_prices.first() {
        println!();
        println!("Best unit price: length {} with {:.2} per unit (price {})",
...
fn main() {
    ...
    run_test_case("Classic Example", vec![1, 5, 8, 9, 10, 17, 17, 20], 8, Some(22));
    ...
    println!("Subproblems per second: {:.0}", (100.0 * 100.0) / (stress_elapsed / 1000.0));
