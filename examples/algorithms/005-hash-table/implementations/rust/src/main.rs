...
impl<K: Hash + Eq + Clone + Debug, V: Clone + Debug> OpenAddressingHashTable<K, V> {
    ...
    fn analyze_distribution(&self) -> DistributionAnalysis {
        ...
        for bucket in &self.buckets {
            if let Some((key, _)) = bucket {
                let ideal_pos = self.hash(key);
                let actual_pos = self.buckets.iter().position(|b| {
                    if let Some((k, _)) = b {
                        std::ptr::eq(k, key)
                    } else {
                        false
        ...
        let variance = if probe_distances.len() > 1 {
            ...
            probe_distances.iter()
                .map(|&d| (d as f64 - mean).powi(2))
                .sum::<f64>() / probe_distances.len() as f64
        } else {
            0.0
        };
    ...
    fn show_stats(&self) {
        ...
        println!("   Average probe distance: {:.2}", analysis.average_probe_distance);
...
impl<K: Hash + Eq + Clone + Debug, V: Clone + Debug> ChainingHashTable<K, V> {
    ...
    fn show_stats(&self) {
        ...
        println!("   Non-empty buckets: {} ({:.1}%)", non_empty_buckets, 
        ...
        println!("   Load factor: {:.3}", self.size as f64 / self.capacity as f64);
    }
}
...
fn benchmark_hash_functions() {
    ...
    println!("   DJB2 hashing time: {:?} for {} keys", djb2_time, test_keys.len());
    println!("   FNV-1a hashing time: {:?} for {} keys", fnv1a_time, test_keys.len());
    ...
    println!("   DJB2 unique hashes: {} / {} ({:.2}% uniqueness)", 
             djb2_unique.len(), test_keys.len(), 
             djb2_unique.len() as f64 / test_keys.len() as f64 * 100.0);
    println!("   FNV-1a unique hashes: {} / {} ({:.2}% uniqueness)", 
             fnv1a_unique.len(), test_keys.len(),
...
fn main() {
    ...
    for (key, value) in &test_data {
        ...
        println!("   Inserted: {} → {} (replaced: {:?})", key, value, old_value);
    }
    ...
    for (key, expected) in test_data.iter().skip(1).take(3) { // Skip apple which was removed
